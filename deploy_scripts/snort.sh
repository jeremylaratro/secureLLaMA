...
dnf update -y
dnf install -y cmake xz-devel dh-autoreconf autoconf automake libtool m4 make \
    gettext pkgconf-pkg-config flex gcc gcc-c++ hwloc hwloc-devel libpcap \
    libpcap-devel pcre pcre-devel luajit luajit-devel libdnet libdnet-devel \
    uuid uuid-devel hyperscan hyperscan-devel flatbuffers flatbuffers-devel \
    jemalloc-devel zlib zlib-devel wget git tar
