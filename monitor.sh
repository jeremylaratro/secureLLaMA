...
snort -D -i $(ifconfig | awk '{print $1}' | grep : | tr -d : | tr -d lo) -c /etc/snort/snort.lua --rule-path /etc/snort/rules --daq-dir /usr/local/lib/daq -l /var/log/snort &
...
sudo chmod +x /usr/local/bin/clam.sh
# Define the cron job
